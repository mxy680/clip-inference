...
if (
    open_clip_model_name is not None
    ...
    def check_model_and_pretrained(model_name: str, pretrained: str):
        ...
        logging.error(
            f"Fatal: Match not found for OPEN_CLIP model {model_name} with pretrained {pretrained} pair"
        )
        sys.exit(1)

    logging.info(
        f"Checking if OPEN_CLIP model {open_clip_model_name} and pretrained {open_clip_pretrained} is a valid pair"
    )
    check_model_and_pretrained(open_clip_model_name, open_clip_pretrained)

    cache_dir = "./models/openclip"
    logging.info(
        f"Downloading OPEN_CLIP model {open_clip_model_name} with pretrained {open_clip_pretrained} to cache dir {cache_dir}"
    )
    model, _, preprocess = open_clip.create_model_and_transforms(
        open_clip_model_name, pretrained=open_clip_pretrained, cache_dir=cache_dir
    )
...
if clip_model_name.startswith("openai/") or clip_model_type.lower() == "openai":
    if clip_model_name != text_model_name:
        ...
        sys.exit(1)
    logging.info(
        "Downloading OpenAI CLIP model {} from huggingface model hub".format(
            clip_model_name
        )
    )
    ...
    logging.info(
        "Downloading text model {} from huggingface model hub".format(text_model_name)
    )
    text_model = SentenceTransformer(text_model_name)
    text_model.save("./models/text")

    logging.info(
        "Downloading img model {} from huggingface model hub".format(clip_model_name)
    )
